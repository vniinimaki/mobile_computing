...
fun BackScreen(
    ...
    var apiCallState by remember {
        mutableStateOf(false)
    }
    var apiCallText: String = "blank"
    ...
    Column(modifier = Modifier.padding(all = 8.dp)){
        ...
        Button(onClick = {
            val call = ApiClient.apiService.getPostById(1)
            call.enqueue(object : Callback<WarningResponse> {
                override fun onResponse(
                    call: Call<WarningResponse>,
                    response: Response<WarningResponse>
                ) {
                    if (response.isSuccessful) {
                        val post = response.body()
                        apiCallText = response.body().toString()
                        apiCallState = true
                ...
                override fun onFailure(call: Call<WarningResponse>, t: Throwable) {
                    ...
                    Log.d("TAG_", "API call failed")
                }
            })
            }) {
                if (!apiCallState) Text("Call API") else Text(apiCallText)
                }
            }
